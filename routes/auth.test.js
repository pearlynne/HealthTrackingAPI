...
describe("Authentication routes", () => {
  ...
  const user1 = {
    ...
    describe("PUT /password", () => {
      it("should return 401", async () => {
        const res = await request(server).put("/auth/password").send(user1);
        expect(res.statusCode).toEqual(401);
      });
    });
    ...
    describe("POST /signup", () => {
      ...
      it("should return 409 Conflict with a repeat signup", async () => {
        let res = await request(server).post("/auth/signup").send(user1);
        expect(res.statusCode).toEqual(200);
        res = await request(server).post("/auth/signup").send(user1);
      ...
      it("should not store raw password", async () => {
        ...
        users.forEach((user) => {
          expect(Object.values(user).includes(user1.password)).toBe(false);
        });
      });
    });
  });
